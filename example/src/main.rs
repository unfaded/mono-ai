...
/// Example of how to build clients for different providers
/// This shows the intended API for when we add more providers
fn build_client_examples() -> Vec<(&'static str, Box<dyn Fn() -> UnifiedAI>)> {
    vec![
        // Current working provider
        ("Ollama (local)", Box::new(|| {
            UnifiedAI::ollama("http://localhost:11434".to_string(), "qwen3-coder:30b".to_string())
        })),
        
        // ("Anthropic Claude", Box::new(|| {
        //     UnifiedAI::anthropic("your-api-key".to_string(), "claude-3-sonnet-20240229".to_string())
        // })),
        // Future providers
        // ("OpenAI", Box::new(|| {
        //     UnifiedAI::openai("your-api-key".to_string(), "gpt-4".to_string())
        // })),
        //
        //
        // ("Google Gemini", Box::new(|| {
        //     UnifiedAI::google("your-api-key".to_string(), "gemini-pro".to_string())
        // })),
    ]
}

async fn chat_stream(mut client: UnifiedAI) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("unified-ai example - type 'exit' to quit");
    ...
    loop {
        ...
        let mut tool_calls: Option<Vec<unified_ai::ToolCall>> = None;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() > 1 {
        match args[1].as_str() {
            "providers" => {
                // Show available provider examples
                println!("Available provider examples:");
                let examples = build_client_examples();
                for (name, _builder) in examples {
                    println!("- {}", name);
                }
                // Future providers will use the same UnifiedAI interface:
                // - UnifiedAI::openai(api_key, model)
                // - UnifiedAI::anthropic(api_key, model) 
                // - UnifiedAI::google(api_key, model)
            }
            "list" => {
            ...
            "pull" => {
                if args.len() > 2 {
                    let client =
                        UnifiedAI::ollama("http://localhost:11434".to_string(), "".to_string());
            ...
            _ => {
                let client =
                    UnifiedAI::ollama("http://localhost:11434".to_string(), args[1].to_string());
        ...
        println!("  cargo run providers              - Show available provider examples");
        ...
        println!("\nDefaulting to interactive chat with qwen3-coder:30b");
